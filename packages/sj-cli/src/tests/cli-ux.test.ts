...
test("unknown commands exit with usage status and corrective guidance", () => {
  ...
  assert.match(result.stderr, /Core commands: plan, next, retry, resume, guide, prepare, validate, explain, trace, doctor, parity, runtime, repo-map, toolchain, usage, eval, init, status, check, specialists, agent-pack, checkpoint, handoff, ui/);
});
