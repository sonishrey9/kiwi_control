...
import { normalizeSpecialistId } from "./specialists.js";
...
export async function validateTargetRepo(
  ...
  if (await pathExists(statePaths.currentPhase)) {
    try {
      ...
      validateSpecialistField(
        issues,
        config,
        currentPhase.nextRecommendedSpecialist as string | undefined,
        "current phase nextRecommendedSpecialist",
        statePaths.currentPhase
      );
      validateMcpPackField(
        issues,
        currentPhase.nextSuggestedMcpPack as string | undefined,
        "current phase nextSuggestedMcpPack",
        statePaths.currentPhase
      );
    } catch (error) {
      issues.push({
  ...
  if (activeRoleHints) {
    ...
    validateSpecialistField(issues, config, activeRoleHints.activeRole, "active role hints activeRole", statePaths.activeRoleHints);
    for (const role of activeRoleHints.supportingRoles) {
      validateSpecialistField(issues, config, role, "active role hints supportingRoles", statePaths.activeRoleHints);
    }
    ...
    validateSpecialistField(
      ...
      "active role hints nextRecommendedSpecialist",
      statePaths.activeRoleHints
    );
    ...
    validateMcpPackField(issues, activeRoleHints.nextSuggestedMcpPack, "active role hints nextSuggestedMcpPack", statePaths.activeRoleHints);
  ...
  if (await pathExists(statePaths.latestCheckpointJson)) {
    try {
      ...
      validateSpecialistField(
        issues,
        config,
        checkpoint.nextRecommendedSpecialist as string | undefined,
        "latest checkpoint nextRecommendedSpecialist",
        statePaths.latestCheckpointJson
      );
      validateMcpPackField(
        issues,
        checkpoint.nextSuggestedMcpPack as string | undefined,
        "latest checkpoint nextSuggestedMcpPack",
        statePaths.latestCheckpointJson
      );
    } catch (error) {
      issues.push({
  ...
  if (await pathExists(statePaths.handoffDir)) {
    ...
    for (const entry of entries.filter((item) => item.endsWith(".json") && !isIgnoredArtifactName(item))) {
      ...
      try {
        ...
        validateSpecialistField(issues, config, handoff.fromRole as string | undefined, "handoff fromRole", fullPath);
        validateSpecialistField(issues, config, handoff.toRole as string | undefined, "handoff toRole", fullPath);
        validateMcpPackField(issues, handoff.recommendedMcpPack as string | undefined, "handoff recommendedMcpPack", fullPath);
  ...
  if (await pathExists(memoryPaths.repoFacts)) {
    try {
      ...
      validateSpecialistField(issues, config, repoFacts.activeRole as string | undefined, "repo-facts activeRole", memoryPaths.repoFacts);
      validateMcpPackField(issues, repoFacts.recommendedMcpPack as string | undefined, "repo-facts recommendedMcpPack", memoryPaths.repoFacts);
    } catch (error) {
      issues.push({
        level: "error",
  ...
  try {
    ...
    if (currentFocus) {
      ...
      validateSpecialistField(issues, config, currentFocus.focusOwnerRole, "current-focus focusOwnerRole", memoryPaths.currentFocus);
      validateSpecialistField(
        issues,
        config,
        currentFocus.nextRecommendedSpecialist,
        "current-focus nextRecommendedSpecialist",
        memoryPaths.currentFocus
      );
      validateMcpPackField(issues, currentFocus.nextSuggestedMcpPack, "current-focus nextSuggestedMcpPack", memoryPaths.currentFocus);
    } else if (inspection.alreadyInitialized) {
      issues.push({
        level: "error",
...
function validateSpecialistField(
  ...
  const canonical = normalizeSpecialistId(config, value);
  if (!canonical) {
    issues.push({
      level: "error",
      message: `${fieldName} references unknown specialist ${value}`,
      filePath
    });
    return;
  }
  if (canonical !== value) {
    issues.push({
      level: "warn",
      message: `${fieldName} still uses legacy alias ${value}; prefer ${canonical}`,
      filePath
    });
  }
}

function validateMcpPackField(
  issues: ValidationIssue[],
  value: string | undefined,
  fieldName: string,
  filePath: string
  ...
  if (normalizeMcpPack(value) !== value) {
    issues.push({
      level: "error",
      message: `${fieldName} references unknown MCP pack ${value}`,
      filePath
    });
  }
}
