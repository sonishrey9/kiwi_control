...
export async function renderCodexBody(repoRoot: string, config: LoadedConfig, context: TemplateContext): Promise<string> {
  const templatePath = path.join(repoRoot, config.routing.surfaces.codex.template);
  const template = await readText(templatePath);
  return renderTemplate(template, contextToValues(context));
}

function contextToValues(context: TemplateContext): Record<string, string> {
  return {
