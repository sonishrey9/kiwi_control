...
export async function renderCopilotBody(repoRoot: string, config: LoadedConfig, context: TemplateContext): Promise<string> {
  const templatePath = path.join(repoRoot, config.routing.surfaces.copilot.template);
