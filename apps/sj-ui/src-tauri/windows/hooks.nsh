...
!macro NSIS_HOOK_POSTINSTALL
  ...
  IfFileExists "$INSTDIR\resources\desktop\cli-bundle\install.ps1" 0 kiwi_postinstall_done
...
kiwi_postinstall_run:
  ClearErrors
  ExecWait '"$R1" -NoProfile -ExecutionPolicy Bypass -File "$INSTDIR\resources\desktop\cli-bundle\install.ps1" -InstallScope "$R0" -PreferredNodePath "$INSTDIR\resources\desktop\node\node.exe"' $R2
...
!macro NSIS_HOOK_PREUNINSTALL
  !insertmacro KIWI_CONTROL_DETERMINE_INSTALL_SCOPE $R0
  IfFileExists "$INSTDIR\resources\desktop\cli-bundle\uninstall.ps1" 0 kiwi_preuninstall_done
...
kiwi_preuninstall_run:
  ClearErrors
  ExecWait '"$R1" -NoProfile -ExecutionPolicy Bypass -File "$INSTDIR\resources\desktop\cli-bundle\uninstall.ps1" -InstallScope "$R0"' $R2
